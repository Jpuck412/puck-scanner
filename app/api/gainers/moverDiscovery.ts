...
function dollarVolumeScore(dollarVolume: number): number {
...
function spreadScore(currentPrice: number, spreadPct: number | null): number {
...
export function buildMoverDiscovery(
  ...
  const velocity1m = price1mAgo
    ? safePct(currentPrice, price1mAgo)
    : null;

  const velocity3m = price3mAgo
    ? safePct(currentPrice, price3mAgo) / 3
    : null;

  const velocity5m = price5mAgo
    ? safePct(currentPrice, price5mAgo) / 5
    : null;
  ...
  /*
    ELITE DEV 4 EXPERIMENT:
    Percent rising controls discovery.
    Volume confirms life only.
    Permission/structure comes later.
  */

  const gainScore =
    100 *
    (
      0.82 * sat(pctChange, 45) +
      0.18 * sat(openMove, 25)
    );

  const velocityScore = hasVelocityData
    ? 100 *
      (
        0.60 * sat(velocity1m as number, 4) +
  ...
  const dollarVolScore = dollarVolumeScore(dollarVolume);
  const spreadScoreValue = spreadScore(currentPrice, spreadPct);
  ...
  return {
    ...
    spreadScore: round(spreadScoreValue, 2),
